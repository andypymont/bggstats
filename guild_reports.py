...
def filter_gcs(dataframe, expansions, min_ratings):
    ...
    expansion_status = (
        True if expansions == "all" else (dataframe["is_expansion"] == expansions)
    )
...
reports = OrderedDict(
    [
        ("top20", {"title": "Top 20 Games", "expansions": 0, "rows": 20}),
        ("top10expansions", {"title": "Top 10 Expansions", "expansions": 1}),
        (
            "bottom10",
            {
...
def listall():
    ...
    all_reports = [
        (name, settings.get("title", "Untitled Report"))
        for (name, settings) in reports.items()
    ]
...
def run(report_name):
    ...
    outputs = [
        settings for (rep, settings) in reports.items() if report_name in ("all", rep)
    ]
    if not outputs:
